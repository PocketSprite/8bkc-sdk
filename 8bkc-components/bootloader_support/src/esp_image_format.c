...
esp_err_t esp_image_load(esp_image_load_mode_t mode, const esp_partition_pos_t *part, esp_image_metadata_t *data)
{
    ...
    err = esp_image_verify_image_header(data->start_addr, &data->image, silent);
    if (err != ESP_OK) {
goto err;
    }
    ...
    if (do_load) { // Need to deobfuscate RAM
        for (int i = 0; i < data->image.segment_count; i++) {
            ...
            if (esp_image_segaddr_should_load(load_addr)) {
...
static esp_err_t process_segment(int index, uint32_t flash_addr, esp_image_segment_header_t *header, bool silent, bool do_load, bootloader_sha256_handle_t sha_handle, uint32_t *checksum)
{
    ...
    bool is_mapping = esp_image_segaddr_should_map(load_addr);
    do_load = do_load && esp_image_segaddr_should_load(load_addr);
...
esp_err_t esp_image_verify_segment_header(int index, const esp_image_segment_header_t *segment, uint32_t segment_data_offs, bool silent)
{
    ...
    bool map_segment = esp_image_segaddr_should_map(load_addr);
...
bool esp_image_segaddr_should_load(uint32_t load_addr)
{
    /* Reload the RTC memory segments whenever a non-deepsleep reset
       is occurring */
    bool load_rtc_memory = rtc_get_reset_reason(0) != DEEPSLEEP_RESET;

    if (esp_image_segaddr_should_map(load_addr)) {
        return false;
    }
